(function () {
    ...
    function renderGame() {
        ...
        if (state.hidden_consequences && state.hidden_consequences.length > 0) {
            renderConsequences();
        }
    }
    ...
    function renderBrainToggle() {
        const btn = document.createElement("button");
        btn.id = "brain-toggle";
        btn.textContent = brainOpen ? "Hide AI Brain" : "Show AI Brain";
        btn.addEventListener("click", (e) => {
            e.stopPropagation();
            brainOpen = !brainOpen;
            render();
    ...
    function renderAIBrain() {
        ...
        title.textContent = "Factor Graph Inference";
        header.appendChild(title);

        const subtitle = document.createElement("div");
        subtitle.className = "brain-subtitle";
        subtitle.textContent = "Belief propagation over hidden variables";
        ...
        varsSection.className = "brain-section";

        const varsTitle = document.createElement("div");
        varsTitle.className = "brain-section-title";
        varsTitle.textContent = "Variable Nodes";
        varsSection.appendChild(varsTitle);

        const vars = state.factor_graph.variables;
        for (const [name, v] of Object.entries(vars)) {
            const row = document.createElement("div");
            row.className = "brain-var-row";

            const label = document.createElement("div");
            label.className = "brain-var-label";
            label.textContent = name.replace(/_/g, " ");
            row.appendChild(label);

            const barContainer = document.createElement("div");
            barContainer.className = "brain-bar-container";

            const barFill = document.createElement("div");
            barFill.className = "brain-bar-fill";
            barFill.style.width = v.mean + "%";
            if (v.mean > 70) barFill.classList.add("high");
            else if (v.mean > 40) barFill.classList.add("mid");
            else barFill.classList.add("low");
            barContainer.appendChild(barFill);

            const uncertainty = document.createElement("div");
            uncertainty.className = "brain-uncertainty";
            const stdPct = Math.min(v.std, 50);
            uncertainty.style.width = stdPct + "%";
            uncertainty.style.left = Math.max(0, v.mean - stdPct / 2) + "%";
            barContainer.appendChild(uncertainty);

            row.appendChild(barContainer);

            const val = document.createElement("div");
            val.className = "brain-var-value";
            val.textContent = v.mean.toFixed(1) + " ±" + v.std.toFixed(1);
            row.appendChild(val);
    ...
    function renderConsequences() {
        if (!state.hidden_consequences || state.hidden_consequences.length === 0) return;

        const panel = document.createElement("div");
        panel.id = "consequences-panel";

        const title = document.createElement("div");
        title.className = "consequences-title";
        title.textContent = "Hidden Consequences Detected";
        panel.appendChild(title);

        for (const c of state.hidden_consequences) {
            const item = document.createElement("div");
            item.className = "consequence-item";

            const msg = document.createElement("div");
            msg.className = "consequence-msg";
            msg.textContent = c.message;
            item.appendChild(msg);

            const prob = document.createElement("div");
            prob.className = "consequence-prob";
            prob.textContent = "Probability: " + c.probability + "%";
