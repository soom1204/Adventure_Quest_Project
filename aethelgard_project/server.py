...
from ai_engine import FactorGraphEngine
from narrative_data import NODES
...
def game_assets(filename):
    directory = os.path.join("assets", os.path.dirname(filename).split("/")[0] if "/" in filename else "")
    fname = os.path.basename(filename) if "/" in filename else filename
    if "backgrounds" in filename:
        return send_from_directory("assets/backgrounds", fname)
    elif "characters" in filename:
        return send_from_directory("assets/characters", fname)
